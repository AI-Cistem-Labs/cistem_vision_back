...
✅ ENVÍA comandos de patrullaje al cliente patrol
...
def handle_connect(auth):
    ...
    client_type = auth.get("client_type", "unknown")  # telemetry | patrol | unknown
...
# ============================================================================
# COMANDOS (SALIDA) -> SIEMPRE al client_type="patrol"
# ============================================================================
def send_command_to_robot(command: str, device_id: int = 1, location_id: int = 1):
    ...
    if patrol_sid is None:
        logger.error(f"❌ No se puede enviar comando: patrol client no conectado ({location_id},{device_id})")
        return {"success": False, "error": "Patrol client no conectado"}
    ...
    try:
        ...
        return {"success": True, "message": "OK", "command_data": command_data}
    except Exception as e:
  ...
  <div class="wrap">
    <div class="topbar">
      <div class="card" style="flex:1">
        ...
        <div class="sub">Telemetría + control de patrullaje en tiempo real (Socket.IO + HTTP)</div>
    ...
    <div class="small" style="margin-top:14px">
      Debug: <a href="/debug/sids" target="_blank">/debug/sids</a> ·
...
<script>
  ...
  async function refreshStatus(){
    try{
      ...
      log(`🔄 Poll status: telemetry=${j.telemetry_connected} patrol=${j.patrol_connected} state=${j.state || 'n/a'}`);
    }catch(e){
      log(`❌ Poll error: ${e}`);
    }
  }
  ...
  socket.on('connect', () => { setWS(true); log('✅ Socket conectado'); refreshStatus(); });
  socket.on('disconnect', () => { setWS(false); log('⚠️ Socket desconectado'); });

  socket.on('robot_info', (data) => {
    // data viene igual que recibes del robot
    setTelemetry(data);
  });
...
if __name__ == "__main__":
    print("\n" + "=" * 70)
    print("🎯 ROBOT COMMANDER - Control Bidireccional Completo (UI integrada)")
    print("=" * 70)
    print("🌐 UI: http://0.0.0.0:5000/")
    print("📡 API: POST /robot/command | GET /robot/status | GET /debug/sids")
    print("✅ Integrado con station_controller (cámaras del robot)")
    print("=" * 70)
